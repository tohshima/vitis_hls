// Register/UART input task module for hackcpu
#include "hackcpu.hpp"
...
void command_in_task(
	...
	#pragma HLS INTERFACE axis port=axireg_command_in depth=20
    ...
    if (!axireg_command_in.empty())  {
        command_in.write(make_command_word(CMDTAG_REG, axireg_command_in.read()));
    }
	if (!uart_in.empty()) {
		...
		if (token0 == 'K') {
			...
			command_in.write(make_command_word(CMDTAG_UART, command));
		}
	}
}
